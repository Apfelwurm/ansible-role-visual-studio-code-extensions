...
def uninstall_extension(module, executable, name, userdatadir):
    if is_extension_installed(module, executable, name, userdatadir):
        if userdatadir != "":
            rc, out, err = module.run_command(
                [executable, '--user-data-dir', userdatadir, '--uninstall-extension', name])
        else:
            rc, out, err = module.run_command(
...
def run_module():
    ...
    if state == 'absent':
        changed = uninstall_extension(module, executable, name, userdatadir)
